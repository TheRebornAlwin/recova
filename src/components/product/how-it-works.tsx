"use client";

import Image from "next/image";
import ScrollReveal from "@/components/ui/scroll-reveal";

const steps = [
  {
    step: 1,
    image: "https://xp3x50z315.ufs.sh/f/4WAjKEfnI5pfEyjo2PN5jIWLlegYskdQVfhE3cGRCDKxNiBZ",
    description: "Place the gel pads on the back of your neck and turn on the device",
  },
  {
    step: 2,
    image: "https://xp3x50z315.ufs.sh/f/4WAjKEfnI5pfE7GgSL5jIWLlegYskdQVfhE3cGRCDKxNiBZw",
    description: "Select your preferred massage mode and intensity level using the controller",
  },
  {
    step: 3,
    image: "https://xp3x50z315.ufs.sh/f/4WAjKEfnI5pf8caa9Abs4qMzpg3kyZeFh0XQWYPCr7wRbN9V",
    description: "Feel your neck pain melt away...",
  },
];

export default function HowItWorks() {
  return (
    <section className="py-20 md:py-28 px-6 bg-white">
      <div className="max-w-4xl mx-auto">
        <ScrollReveal>
          <div className="text-center mb-14">
            <h2 className="text-2xl md:text-3xl font-black text-navy tracking-tight uppercase">
              How to Use Recova&trade;?
            </h2>
          </div>
        </ScrollReveal>

        <div className="grid grid-cols-3 gap-4 sm:gap-8">
          {steps.map((s, i) => (
            <ScrollReveal key={i} delay={i * 0.1} className="min-w-0">
              <div className="text-center">
                {/* Circular image */}
                <div className="w-28 h-28 sm:w-40 sm:h-40 md:w-52 md:h-52 rounded-full overflow-hidden mx-auto border-2 border-gray-100 shadow-sm">
                  <Image
                    src={s.image}
                    alt={`Step ${s.step}`}
                    width={250}
                    height={250}
                    className="w-full h-full object-cover"
                  />
                </div>

                {/* Step label */}
                <p className="text-sm sm:text-base font-black text-navy uppercase tracking-wide mt-5 mb-2">
                  Step {s.step}
                </p>

                {/* Description */}
                <p className="text-xs sm:text-sm text-slate leading-relaxed italic">
                  {s.description}
                </p>
              </div>
            </ScrollReveal>
          ))}
        </div>
      </div>
    </section>
  );
}
